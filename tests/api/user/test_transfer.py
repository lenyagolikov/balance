...
def test_transfer_success(client: TestClient, users_in_db, body: dict):
...
def test_transfer_sender_not_found(client: TestClient, users_in_db, body: dict):
    resp = client.post(f"/{prefix}/transfer", json=body)
...
def test_transfer_receiver_not_found(client: TestClient, users_in_db, body: dict):
    resp = client.post(f"/{prefix}/transfer", json=body)
    assert resp.status_code == status.HTTP_400_BAD_REQUEST
...
def test_transfer_not_money(client: TestClient, users_in_db, body: dict):
    resp = client.post(f"/{prefix}/transfer", json=body)
    assert resp.status_code == status.HTTP_400_BAD_REQUEST
