...
table in db -> transactions: operations history

query params: offset + limit (pagination), desc-amount (sorting amount), desc-date (sorting date)

1. create record in transactions (deposit, withdraw, transfer)
2. get transactions: show ALL transactions

Schema for request:
    offset: int | None = 1
    limit: int | None = 10
    desc-amount: bool | None = false
    desc-date: bool | None = false
...
1. Model for Transaction
    pkid type from to amount comment timestamp
    int  str int  int decimal str timestamp

2. Schemas for TransactionRequest, TransactionResponse
    look above

3. CRUD for Transaction
    - get transanction
    - create transaction

4. Handler and routing
